...
```{r}
avg_steps_perday <- as.data.table(aggregate(steps ~ date,na.rm = TRUE, data=activity, FUN=mean))

activity_old <- copy(activity)

activity <- activity[which(is.na(activity$steps)), steps:= avg_steps_perday[which(avg_steps_perday$date==date)]$steps]
